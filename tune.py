...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        const=1,
        default=1000000,
        help="Number of training steps for the agent",
    )
    parser.add_argument(
        "--debug",
        action="store_false",
    ...
    parser.add_argument(
        "--overlay", type=str, help="Path to overlay graph for the environment"
    )
    parser.add_argument(
        "--requests",
        type=str,
    ...
    parser.add_argument(
        "--agent", type=str, help="Whether to use a RL agent or a baseline"
    )
    parser.add_argument(
        "--logs",
        type=str,
    ...
    parser.add_argument(
        "--trials", type=int, default=10, help="Number of trials evaluating the agent"
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    for trial in range(args.trials):
        ...
        def evaluate_objective(config):
            ...
            tune_agent.learn(
                total_timesteps=args.sample_timesteps, callback=monitor_callback
            )
        ...
        asha_scheduler = ASHAScheduler(
            time_attr="training_iteration", metric="episode_reward_mean", mode="max"
        )
        ...
        tb_log_name = (
            eval_agent.__class__.__name__
            if isinstance(policy, str)
            else policy.__name__
        )
        eval_agent.learn(
            total_timesteps=args.total_train_timesteps,
        ...
        results[trial] = evaluate_final_policy(args.eval_episodes, eval_agent, env)
